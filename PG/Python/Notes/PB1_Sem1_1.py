...
def monkey_trouble(a_smile, b_smile):
    return (False if not a_smile and b_smile or a_smile
            and not b_smile else True)
...
def combo_string(a, b):
    x = ('{}{}{}'.format(b, a, b) if len(a)
         > len(b) else '{}{}{}'.format(a, b, a))
    return x
...
def caught_speeding(speed, is_birthday):
    if is_birthday == False:
        return (0 if speed <= 60 else (1 if 61 <= speed <= 80 else 2))  # if speed > 81
    else:
        return (0 if speed <= 65 else (1 if 66 <= speed <= 85 else 2))  # if speed > 86
...
def near_ten(num):
    return (True if 0 <= num % 10 <= 2 or 8 <= num % 10
            <= 10 else False)


# CodingBat Logic2 - make_bricks
def make_bricks(small, big, goal):
    return (False if goal > sum((big * 5, small)) else (False if goal
            % 5 > small else True))
...
def lucky_sum(a, b, c):
    return (0 if a == 13 else (a if b == 13 else (a + b if c
            == 13 else sum((a, b, c)))))
...
def end_other(a, b):
    al = len(a)
    bl = len(b)
    return (True if b.lower()[-bl:] in a.lower()[-bl:]
            or a.lower()[-al:] in b.lower()[-al:] else False)
