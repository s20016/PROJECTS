# ------------------------------- Version Notes ------------------------------ #
# [Project 5] [School Project] Yasumi Desuka? App. v2
...
import webbrowser, random, os, time
...
def App_answer(user_response, yn_img, ans, ans2, ans3):
    ...
    event_style = f"h2 {{padding-top: 15rem; font-size: 37px; background: transparent;}}"

    html_content = f"<html><head>\n<style>{img_style}\n{ans_style}\n{emoji_style}\n{event_style}</style>\n<title>休みですか？</title></head>\n<body><h2>{ans3}</h2><h1>{ans}<br><span>{x}{y}{z}</span></h1></body>\n</html>"
...
if user_input in holidays_jp.keys():
    ...
    file_name = App_answer(user_input, yn_response_img, ans = response_output, ans2 = emoji_output, ans3 = event)
else:
    response_output, emoji_output = "休みじゃない！！", "no"
    yn_response_img = Correct_response(emoji_output)
    file_name = App_answer(user_input, yn_response_img, ans = response_output, ans2 = emoji_output, ans3 = "")
